...
component_1.VantComponent({
  ...
  methods: {
      ...
      var lists = fileList.map(function (item) {
        return __assign(__assign({}, item), {
          isImage: typeof item.isImage === 'undefined' ?
            utils_1.isImageFile(item) : item.isImage,
    ...
    deleteItem: function (event) {
      ...
      this.$emit('delete',__assign(__assign({}, this.getDetail(index)), {
    ...
    onPreviewImage: function (event) {
      ...
      wx.previewImage({
        urls: lists
          ...
          .map(function (item) {
            return item.url || item.path || item.tempFilePath;
          }),
        current: item.url || item.path  || item.tempFilePath,
        fail: function () {
          wx.showToast({
            title: '预览图片失败',
            icon: 'none'
